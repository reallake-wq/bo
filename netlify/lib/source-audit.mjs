...
function extractStockCode(...values) {
  ...
  return Array.from(new Set(text.match(/(?<!\d)(?:60|68|00|30|83|87|43|92)\d{4}(?!\d)|\b[A-Z]{1,5}\b/g) || []))[0] || "";
}
...
function sourceScore(source) {
  ...
  if (source.sourceType === "主体核对来源") score += 25;
...
export function auditSources(sources = [], context = {}) {
  ...
  let deduped = dedupeSimilarSources(cleaned).slice(0, context.max || 80);
  ...
  if (deduped.length < minSources) {
    ...
    if (weak.length) deduped = dedupeSimilarSources([...deduped, ...weak]).slice(0, context.max || 80);
  }
  const warnings = [];
